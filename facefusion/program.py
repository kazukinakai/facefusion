...
def create_frame_extraction_program() -> ArgumentParser:
	...
	group_frame_extraction.add_argument('--resume', help = translator.get('help.resume'), action = BooleanOptionalAction, default = config.get_bool_value('frame_extraction', 'resume', 'True'))
	job_store.register_step_keys([ 'trim_frame_start', 'trim_frame_end', 'temp_frame_format', 'keep_temp', 'resume' ])
	return program
...
def create_output_creation_program() -> ArgumentParser:
	...
	group_output_creation.add_argument('--output-file-naming', help = translator.get('help.output_file_naming'), default = config.get_str_value('output_creation', 'output_file_naming', 'target_index'), choices = facefusion.choices.output_file_namings)
	job_store.register_step_keys([ 'output_image_quality', 'output_image_scale', 'output_audio_encoder', 'output_audio_quality', 'output_audio_volume', 'output_video_encoder', 'output_video_preset', 'output_video_quality', 'output_video_scale', 'output_video_fps', 'output_file_naming' ])
	return program
...
def create_execution_program() -> ArgumentParser:
	...
	group_execution.add_argument('--tensorrt-fp16', help = translator.get('help.tensorrt_fp16'), action = BooleanOptionalAction, default = config.get_bool_value('execution', 'tensorrt_fp16', 'False'))
	job_store.register_job_keys([ 'execution_device_ids', 'execution_providers', 'execution_thread_count', 'tensorrt_fp16' ])
	return program
...
def create_program() -> ArgumentParser:
	...
	job_split_program = sub_program.add_parser('job-split', help = translator.get('help.job_split'), parents = [ create_job_id_program(), create_jobs_path_program(), create_log_level_program() ], formatter_class = create_help_formatter_large)
	job_split_program.add_argument('--step-frame-total', help = translator.get('help.step_frame_total'), type = int, required = True)
