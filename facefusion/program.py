...
def create_program() -> ArgumentParser:
	...
	job_optimize_program = sub_program.add_parser('job-optimize', help = translator.get('help.job_optimize'), parents = [ create_job_id_program(), create_jobs_path_program(), create_log_level_program() ], formatter_class = create_help_formatter_large)
	job_optimize_program.add_argument('--step-frame-total', help = translator.get('help.step_frame_total'), type = int, dest = 'step_frame_total', required = True)
