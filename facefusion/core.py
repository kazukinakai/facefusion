...
def route(args : Args) -> None:
	...
	if state_manager.get_item('command') in [ 'job-list', 'job-create', 'job-submit', 'job-submit-all', 'job-delete', 'job-delete-all', 'job-add-step', 'job-remix-step', 'job-insert-step', 'job-remove-step', 'job-split' ]:
...
def route_job_manager(args : Args) -> ErrorCode:
	...
	if state_manager.get_item('command') == 'job-split':
		if job_manager.split_job(state_manager.get_item('job_id'), args.get('step_frame_total')):
			logger.info(translator.get('job_split').format(job_id = state_manager.get_item('job_id'), step_total = job_manager.count_step_total(state_manager.get_item('job_id'))), __name__)
			return 0
		logger.error(translator.get('job_not_split').format(job_id = state_manager.get_item('job_id')), __name__)
		return 1
	return 1
